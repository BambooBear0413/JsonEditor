...
public class ResourceImageLoader {
	...
	public static void loadImages() {
		try(BufferedReader br = new BufferedReader(new InputStreamReader(ClassLoader.getSystemResourceAsStream(IMAGE_LIST)))) {
			String name;
	...
	public static Image getImage(String name) {
		...
		if(image == null) image = loadImage(url);
		
		return image;
	}
	
	public static Image getIconImage(String name) { return getImage("icon/" + name); }
	...
	public static ImageIcon getImageIcon(String name, int width, int height, int hints) {
		...
		} catch (NullPointerException e) { image = null; }
